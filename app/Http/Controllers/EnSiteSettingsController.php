...
class EnSiteSettingsController extends Controller
{
    public function index(){
        ...
        return view('backend.settings.en_site_settings', compact('siteSettings'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('logo')) {
            $filename = 'en-logo.' . $request->file('logo')->getClientOriginalExtension();
            $request->file('logo')->move(config('appconfig.commonPath'), $filename);
            $site_settings->logo = $filename;
        }

        if ($request->hasFile('ogImage')) {
            $filename = 'en-og-default.' . $request->file('ogImage')->getClientOriginalExtension();
            $request->file('ogImage')->move(config('appconfig.commonPath'), $filename);
